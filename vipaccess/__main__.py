...
def provision(p, args):
    ...
    if args.print:
        ...
        if otp_token['period'] is not None and otp_token['counter'] is None:
            ...
            c = otp_token['counter']
            print(
                '    oathtool    {}-c{} -b --hotp {}  # output next code (need to increment counter each time!)'''.format(
                    d, c, otp_secret_b32))
...
def keychain(p, args):
    print("Generating OTP URL & QR code from keychain...")
    kc.get_credential_from_keychain()
...
def check(p, args):
    ...
    session = vp.requests.Session()
    for skew in (None, +d['period'] // 2, -d['period'] // 2, +d['period'], -d['period'], +d['period'] * 3 // 2,
                 -d['period'] * 3 // 2):
        if skew is None:
...
def main():
    ...
    m.add_argument('-o', '--dotfile', type=PathType(type='file', exists=False),
    ...
    m.add_argument('-f', '--dotfile', type=PathType(type='file', exists=True),
                   default=os.path.expanduser('~/.vipaccess'),
    ...
    pkeychain = sp.add_parser('keychain', help='Generate otp url & QRCode from VIP Access KeyChain')
    pkeychain.set_defaults(func=keychain)
